...
public class CoopController {
    ...
    public CoopDto createCoop(CoopDto coopDto) {
    	...
    	CourseOffering courseOffering = courseOfferingService.getCourseOfferingById(courseOfferingDto.getId());
    ...
    public CoopDto updateCoop(@RequestBody CoopDto coopDto) {
    	...
    	if (employerReportDtos != null) {
    		for (EmployerReportDto employerReportDto : employerReportDtos) {
    			EmployerReport employerReport = employerReportService.getEmployerReport(employerReportDto.getId());
    	...
    	if (employerReportDtos != null) {
    		for (StudentReportDto studentReportDto : studentReportDtos) {
    			StudentReport studentReport = studentReportService.getStudentReport(studentReportDto.getId());
    			studentReports.add(studentReport);
    		}
    	}
    	
    	coop = coopService.updateCoop(coop, 
    								  coopDto.getStatus(), 
    								  courseOffering, student, 
    								  coopDetails, employerReports, 
    								  studentReports);
    	return ControllerUtils.convertToDto(coop);
    }
